...
export default function VendorModal({ open, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-1 z-50 flex items-start justify-center">
      {/* backdrop */}
      <div className="absolute inset-0 bg-black/30" onClick={onClose} />
      {/* card */}
      <div className="relative bg-white w-[98%] max-w-5xl mt-6 rounded-2xl shadow-2xl p-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xl md:text-2xl font-semibold text-emerald-700">New Vendor</h3>
          <button onClick={onClose} className="px-3 py-1 rounded-xl bg-gray-100 hover:bg-gray-200">✕</button>
        </div>
          ...
          {tab === "Other Details" && (
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm text-gray-700 mb-2">Documents (max 10 files, 10MB each)</label>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button onClick={save} className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white">Save</button>
        </div>
      </div>
    </div>
  );
